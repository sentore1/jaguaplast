...
import type { FooterLink } from "@/lib/types/cms";
...
export default function ContactClient({ footerLinks, settings }: Props) {
  ...
  const contactInfo = [
    { icon: Mail,    label: "Email",         value: settings.company_email    || "info@jaguaplast.com",    sub: settings.email_reply_time      || "We reply within 1 business day" },
    { icon: Phone,   label: "Phone",         value: settings.company_phone    || "0788882888",              sub: settings.working_hours_weekday || "Mon – Fri, 9 AM – 5 PM" },
    { icon: MapPin,  label: "Address",       value: settings.company_address_line1 || "Kigali, Rwanda",    sub: settings.company_address_line2 || "Industrial Area, Masoro" },
    { icon: Clock,   label: "Working Hours", value: settings.working_hours_weekday || "Mon – Fri: 9 AM – 5 PM", sub: settings.working_hours_weekend || "Saturday: 9 AM – 5 PM" },
  ];
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="px-6 py-24 md:px-12 md:py-32 lg:px-20" style={{ backgroundColor: "#0B7380" }}>
        <div className="mx-auto max-w-5xl">
          <div className="mb-16">
            <p className="mb-4 text-xs uppercase tracking-[0.3em] text-white">Send a Message</p>
          ...
          {submitted ? (
            <div className="border border-white/10 p-12 text-center">
              ...
              <p className="mt-3 text-sm text-white/50">Your message has been received. We&apos;ll be in touch shortly.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="flex flex-col gap-2">
                <label className="text-xs uppercase tracking-widest text-white" htmlFor="name">Full Name</label>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs uppercase tracking-widest text-white" htmlFor="email">Email Address</label>
                <input id="email" name="email" type="email" required value={form.email} onChange={handleChange}
                  className="border-b border-white/20 bg-transparent pb-3 text-sm text-white placeholder:text-white/20 focus:border-white/60 focus:outline-none"
                  placeholder="john@company.com" />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-xs uppercase tracking-widest text-white" htmlFor="phone">Phone Number</label>
                <input id="phone" name="phone" type="tel" value={form.phone} onChange={handleChange}
                  className="border-b border-white/20 bg-transparent pb-3 text-sm text-white placeholder:text-white/20 focus:border-white/60 focus:outline-none"
                  placeholder="+250 788 306 799" />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-xs uppercase tracking-widest text-white" htmlFor="subject">Subject</label>
              ...
              <div className="flex flex-col gap-2 md:col-span-2">
                <label className="text-xs uppercase tracking-widest text-white" htmlFor="message">Message</label>
              ...
              <div className="md:col-span-2 flex flex-col gap-4">
                {error && (
                  <p className="text-sm text-red-300 border border-red-300/30 bg-red-500/10 px-4 py-3">{error}</p>
                )}
                <div>
                  <button type="submit" disabled={loading}
                    className="inline-flex items-center gap-2 bg-white px-8 py-3.5 text-sm font-medium text-foreground transition-opacity hover:opacity-80 disabled:opacity-60">
